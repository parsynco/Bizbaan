...
namespace Parsyn.Apps.Company.Services.Services.Bizbaan
{
    public class AdCategoryService(PDBContext ctx, ILogger log) : BaseService<AdCategoryModel>(ctx, log), IAdCategoryIface
    {
        ...
        {
            return [.. _dbObj.Include(x => x.Parent).Include(x => x.Seo)];
        }

        public List<AdCategoryModel> GetHierichal()
        {
            return [.. _dbObj.Where(x => x.ParentId == null)?.Include(x => x.Childs).ThenInclude(x => x.Childs)];
        }
        ...
        {
            var cats = _dbObj.Where(x => x.ParentId == null).Include(x => x.Seo).ToList();
            return cats;
        }

        public List<AdCategoryModel> GetLevelTwo()
        {
            var cats = _dbObj.Where(x => x.ParentId != null).Include(x => x.Seo).Include(x => x.Parent).ThenInclude(x => x.Seo).ToList();
            return cats;
        }
    }
}
