...
namespace Parsyn.Apps.Api.Controllers.Admin
{
    ...
    public class FormController(IFormsIface srvc, ISubscribeIface subscribe) : BizApiController
    {
        IFormsIface _srvc = srvc;
        ISubscribeIface _subscribe = subscribe;
        private RespGeneratorSrvc _rspgen = new RespGeneratorSrvc();

        [HttpGet]
        [EndpointName("FORMS GET ALL")]
        public ActionResult GetAll()
        {
            var result = _srvc.GetAll();
            return Content(JsonConvert.SerializeObject(result));
        }
        [HttpGet]
        [EndpointName("SUBSCRIBE GET ALL")]
        [Route("Subscribe")]
        public ActionResult GetSubscribs()
        {
            var result = _subscribe.GetAll();
            return Content(JsonConvert.SerializeObject(result));
        }
        //[HttpPost("GetById")]
        ...
        {
            var opr = _subscribe.Delete(x => x.Id == Id);
            return new JsonResult(opr);
        }
    }
}
